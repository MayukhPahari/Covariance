...
def user_def_input():
    ...
    while(True):
        f_high = float(input("Enter high frequency (Nyquist frequency): "))
        if not isinstance(f_high, float):
            raise Exception("IncorrectDataType: Frequency must be float or int")
        if f_high > f_max:
            warning = f"Nyquist frequency must be <= {np.format_float_scientific(f_min, precision=4)}"
...
def calc_covar_spectra():
    ...
    for enr in range(13):
        ...
        err_arr[enr] = covar_err
    
    return covar_arr, excess_cov_arr, err_arr

cov, excess_cov, err = calc_covar_spectra()
...
plt.errorbar(energy, cov/energy, yerr=err, linestyle="", fmt=".", label="Cov")
plt.errorbar(energy, 3*np.sqrt(excess_cov)/energy, yerr=err, linestyle="", fmt="o", label="rms")
plt.yscale("log")
plt.xscale("log")
#plt.ylim(8e-2, 1e1)
plt.legend()
